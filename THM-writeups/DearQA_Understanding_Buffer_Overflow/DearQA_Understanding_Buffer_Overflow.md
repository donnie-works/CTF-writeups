...
- **Main Memory:** Functions as short-term memory, temporarily holding data for quick access. The stack is like a to-do list, organizing tasks and remembering what’s in progress.
- **Heap:** Mirrors long-term memory, storing information dynamically for later use.
...
`rip` -  instruction pointer; think "where to now?"
...
`objdump` is another utility common on Linux systems for displaying various information about object files - here we are using the `-t` switch to dump symbol information.
...
Also, notice they bother come from the `.text` (i.e. executable) section of the program's code. 
...
`-d` : disassemble
`-M` : options (we specify intel syntax)
...
So our simple program is vulnerable to buffer overflow due to a lack of boundary checking. 
...
For example, you can use commands like `step` to move through source code or `stepi` to step through individual assembly instructions, giving you a detailed view of what's happening under the hood.
...
Here's what happens when `rip` points to "step 4".
...
The stack works on a 'Last In, First Out' (LIFO) principle. Imagine a stack of dishes: it seems intuitive to take the top dish off first, right?
...
Good question, but the answer is no. The `ret` instruction fails because `rsp` (the stack pointer) is now pointing to an invalid memory location. You can think of it like spatial disorientation for the program—it loses its sense of direction and then crashes into a wall.
